"""Shared helpers for the Wk3 probability-distributions notebooks.

Plain module (not jupytext-paired, not matched by create_student_versions.py's
`wk3_*_soln.py` glob) so the load step doesn't need to be repeated as a
solution block in every notebook.

Every notebook imports this module directly (`from _wk3_common import
...
def load_disney_df() -> pl.DataFrame:
    """Load the synthetic Disney+ dataset used throughout the Wk3 notebooks.
    ...
    notebook runs from `output/wk3/` and the dataset lives in a sibling
